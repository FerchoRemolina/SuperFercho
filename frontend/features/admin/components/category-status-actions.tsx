...
export function CategoryStatusActions({
  category,
  compact = false,
}: {
  category: Category;
  ...
  const next = category.status === "ACTIVE" ? "deactivate" : "activate";
      ...
      {confirming === null ? (
        ...
        <div className="grid gap-2 rounded-xl border border-sf-border bg-sf-bg p-3">
          ...
          <p className="text-sm text-sf-muted">
            {confirming === "deactivate"
              ? "Dejará de verse en el catálogo público y sus productos no se podrán vender."
              : `La categoría pasará a estado ${categoryStatusLabel("ACTIVE")}.`}
          </p>
