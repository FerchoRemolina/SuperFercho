...
export function AdminCategoryForm({
  ...
  pending: boolean;
  error: string | null;
  onChange: (values: AdminCategoryFormValues) => void;
  onSubmit: () => void;
  onCancel: () => void;
  validate: (values: AdminCategoryFormValues) => AdminCategoryFieldErrors;
}) {
  const formId = useId();
  const [fieldErrors, setFieldErrors] = useState<AdminCategoryFieldErrors>({});

  function update<K extends keyof AdminCategoryFormValues>(
    key: K,
