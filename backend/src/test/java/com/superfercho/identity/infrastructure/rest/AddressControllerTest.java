...
class AddressControllerTest {
    ...
    void shouldAddAddress() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/addresses")
                ...
                .andExpect(header().string("Location", Matchers.endsWith("/api/v1/addresses/" + ADDRESS_ID)))
        ...
        verifyNoInteractions(
                listAddressesUseCase, updateAddressUseCase, deactivateAddressUseCase, setDefaultAddressUseCase);
    }

    @Test
    void shouldListAddresses() throws Exception {
        when(listAddressesUseCase.execute()).thenReturn(List.of(addressResult(true, AddressStatus.ACTIVE)));
        ...
        verifyNoInteractions(
                addAddressUseCase, updateAddressUseCase, deactivateAddressUseCase, setDefaultAddressUseCase);
    }
    ...
    void shouldUpdateAddress() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/addresses/{addressId}", ADDRESS_ID)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        verify(updateAddressUseCase)
                .execute(new UpdateAddressCommand(
                        ...
                        "3009876543"));
        verifyNoInteractions(
                addAddressUseCase, listAddressesUseCase, deactivateAddressUseCase, setDefaultAddressUseCase);
    }

    @Test
    void shouldDeactivateAddress() throws Exception {
        when(deactivateAddressUseCase.execute(new DeactivateAddressCommand(ADDRESS_ID)))
        ...
        verify(deactivateAddressUseCase).execute(new DeactivateAddressCommand(ADDRESS_ID));
    ...
    void shouldSetDefaultAddress() throws Exception {
        when(setDefaultAddressUseCase.execute(new SetDefaultAddressCommand(ADDRESS_ID)))
                .thenReturn(addressResult(true, AddressStatus.ACTIVE));

        mockMvc.perform(post("/api/v1/addresses/{addressId}/default", ADDRESS_ID))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(ADDRESS_ID.toString()))
                .andExpect(jsonPath("$.isDefault").value(true));

        verify(setDefaultAddressUseCase).execute(new SetDefaultAddressCommand(ADDRESS_ID));
        verifyNoInteractions(
                addAddressUseCase, listAddressesUseCase, updateAddressUseCase, deactivateAddressUseCase);
    }

    @Test
    void shouldMapUnauthenticatedToUnauthorized() throws Exception {
        when(listAddressesUseCase.execute()).thenThrow(new UnauthenticatedUserException());
    ...
    void shouldMapUserNotFoundToNotFound() throws Exception {
        when(listAddressesUseCase.execute()).thenThrow(new UserNotFoundException(USER_ID));
    ...
    void shouldMapAddressOwnershipToNotFoundWithoutDedicatedCode() throws Exception {
        when(updateAddressUseCase.execute(any())).thenThrow(new AddressOwnershipException(USER_ID, ADDRESS_ID));
