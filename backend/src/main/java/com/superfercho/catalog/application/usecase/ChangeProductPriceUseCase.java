...
public final class ChangeProductPriceUseCase {

    private final ProductRepository productRepository;
    private final Clock clock;

    public ChangeProductPriceUseCase(ProductRepository productRepository, Clock clock) {
        this.productRepository = productRepository;
        this.clock = clock;
    }

    public ProductResult execute(ChangeProductPriceCommand command) {
        Product product = productRepository
                .findById(command.productId())
                .orElseThrow(() -> new ProductNotFoundException(command.productId()));
        return ProductResult.from(
                productRepository.save(product.changePrice(command.price(), clock.instant())));
    }
}
